...
public class MapleCharacter extends AnimatedMapleMapObject implements Serializable {
    ...
    private String chalktext, BlessOfFairy_Origin, BlessOfEmpress_Origin, teleportname;
...
//    private long exp;
    private Point old;
    private CharacterPO charPo; // 角色信息
    ...
    public static MapleCharacter getDefault(MapleClient client, Byte[] stat) {
        ...
        ret.charPo.setJob(0);
        ret.charPo.setMeso(0);
        ret.charPo.setLevel(1);
        ret.remainingAp = 0;
        ret.charPo.setFame(0);
        ret.love = 0;
        ret.charPo.setAccount(client.getAccountPo());
...
//    }
    ...
    public static MapleCharacter loadCharFromDB(int charid, MapleClient client, boolean channelserver) {
        ...
        try {
            ...
            ret.mount = new MapleMount(ret, 0, PlayerStats.getSkillByJob(1004, ret.charPo.getJob()), (byte) 0, (byte) 1, 0);
            ...
            if ((channelserver) && (client != null)) {
                MapleMapFactory mapFactory = ChannelServer.getInstance(client.getChannel()).getMapFactory();
                ret.map = mapFactory.getMap(ret.charPo.getMap());
                if (ret.map == null) {
                    MapleLogger.info("不存在的地图："+ret.charPo.getMap());
                ...
                if (partyid >= 0) {
                    MapleParty party = WrodlPartyService.getInstance().getParty(partyid);
                    if ((party != null) && (party.getMemberById(charid) != null)) {
                        ret.party = party;
                    }
                }
...
//                }
            ...
            if (channelserver) {
                ...
                AccountsPO account = ret.getClient().getAccountPo();
                ...
                while (rs.next()) {
                    ...
                    if ((skl > skil.getMaxLevel())) {
                        if ((skil.canBeLearnedBy(ret.charPo.getJob())) && (!skil.isSpecialSkill()) && (!skil.isAdminSkill())) {
                ...
                // @TODO 这里还不知道是啥？
                int maxlevel_ = 0;
                List<CharacterPO> charList = charDao.getCharacterByAccountId(ret.client.getAccID());
                ...
                if (ret.BlessOfFairy_Origin == null) {
                    ret.BlessOfFairy_Origin = ret.charPo.getName();
                }
...
//                }
    ...
    public static void saveNewCharToDB(MapleCharacter chr) {
        ...
        CharacterDao charDao = new CharacterDao();
        try {
            charDao.transactionStart();

            charDao.saveWithoutTransaction(chr.getCharPo());
            int charId = chr.getCharPo().getId();
            ...
            ItemLoader.装备道具.saveItems(itemsWithType, charId);
            charDao.transactionCommit();
        } catch (SQLException | DatabaseException e) {
            MapleLogger.error("save chr data error:", e);
                charDao.transactionRollback();
        }
    }
        ...
        try {
            ...
            if ((!fromcs) && (this.map != null)) {
                if (this.map.getId() == 180000001) {
                    this.charPo.setMap(180000001);
                } else if ((this.map.getForcedReturnId() != 999999999) && (this.map.getForcedReturnMap() != null)) {
                    this.charPo.setMap(this.map.getForcedReturnId());
                } else {
                    this.charPo.setMap(this.stats.getHp() < 1 ? this.map.getReturnMapId() : this.map.getId());
                }
            }
            if (this.map == null) {
                this.charPo.setSpawnpoint((byte)0);
            } else {
                MaplePortal closest = this.map.findClosestSpawnpoint(getTruePosition());
                this.charPo.setSpawnpoint((byte) (closest != null ? closest.getId() : 0));
            }
            this.charPo.setParty(this.party == null ? -1 : this.party.getId());
            this.charPo.setBuddyCapacity(this.buddylist.getCapacity());
            ...
            this.keylayout.saveKeys(this.getId());

            this.quickslot.saveQuickSlots(this.getId());

            this.mount.saveMount(this.getId());
    ...
    private void deleteWhereCharacterId(Connection con, String sql) throws SQLException {
        deleteWhereCharacterId(con, sql, this.getId());
    }
                            ...
                            for (MapleSummon summon : this.summons.values()) {
                                if ((summon.getSkillId() == summonId)|| (((summon.getSkillId() == summonId + 999) || (((summonId == 1085) || (summonId == 1087) || (summonId == 1090)) && (summon.getSkillId() == summonId - 999))))) {
                                    ...
                                    /*if (summon.getSkillId() == 22171052) {
                                     client.sendPacket(MaplePacketCreator.spawnDragon(this.dragon));
                                     }*/
                                }
                            }
                        } finally {
...
//        }
    ...
    private void cancelPlayerBuffs(List<MapleBuffStat> buffstats, boolean overwrite) {
        ...
        if (write) {
            ...
            this.stats.recalcLocalStats(this);
            this.client.sendPacket(BuffPacket.cancelBuff(buffstats, this));
    ...
    public void doDragonBlood() {
        ...
        if (this.stats.getHp() - bloodEffect.getStr() <= 1) {
            ...
            this.client.sendPacket(MaplePacketCreator.showOwnBuffEffect(bloodEffect.getSourceId(), 8, getLevel(), bloodEffect.getLevel()));//7+1 119
    ...
    public void addPowerCount(int delta) {
        ...
        if (setPowerCount(getPowerCount() + delta)) {
            this.stats.recalcLocalStats(this);
            this.client.sendPacket(BuffPacket.updatePowerCount(30020232, Math.min(getPowerCount(), 20)));
        }
    }
    ...
    public boolean isUsePower() {
        this.client.sendPacket(BuffPacket.startPower(this.usePower, this.batterytime));
    ...
    public void handleOrbgain() {
        ...
        if (orbcount < ceffect.getX() + 1) {
            ...
            this.client.sendPacket(BuffPacket.giveBuff(combos.getId(), duration, stat));
    ...
    public int getId() {
        return this.charPo.getId();
    }

    public String getName() {
        return this.getCharPo().getName();
    }
    ...
    public short getLevel() {
        return this.getCharPo().getLevel();
    }

    public int getFame() {
        return this.getCharPo().getFame();
    }
    ...
    public short getHpApUsed() {
        return this.getCharPo().getHpApUsed();
    }
    ...
    public void setHpApUsed(short hpApUsed) {
        this.getCharPo().setHpApUsed(hpApUsed);
    }

    public byte getSkinColor() {
        return this.getCharPo().getSkincolor();
    }

    public void setSkinColor(byte skinColor) {
        this.getCharPo().setSkincolor(skinColor);
    }

    public short getJob() {
        return this.getCharPo().getJob();
    }

    public byte getGender() {
        return this.getCharPo().getGender();
    }
    ...
    public void setExp(long exp) {
        ...
        this.getCharPo().setExp(exp);
    }

    public void addExp(long exp) {
        long totalExp = this.getExp() + exp;
        this.setExp(totalExp);
    }

    public void setFame(int fame) {
        this.getCharPo().setFame(fame);
    }
    ...
    public void setGender(byte gender) {
        this.getCharPo().setGender(gender);
    }
    ...
    public void addFame(int famechange) {
        int totalFame = this.getCharPo().getFame() + famechange;
        this.getCharPo().setFame(totalFame);
    }

    public void updateFame() {
        updateSingleStat(MapleStat.人气, this.getCharPo().getFame());
    }
...
//                updateSingleStat(MapleStat.AVAILABLESP, this.remainingSp);
    ...
    public void baseSkills() { //TODO 添加基础技能
        ...
        Iterator i$;
        if (GameConstants.getJobNumber(this.getJob()) >= 3) {
            List baseSkills = SkillFactory.getSkillsByJob(this.getJob());
            if (baseSkills != null) {
        ...
        if ((this.getJob() >= 432) && (this.getJob() <= 434)) {
    ...
    public void changeSkillLevel_Skip(Map<Skill, SkillEntry> skill, boolean write) {
        ...
        if ((write) && (!newlist.isEmpty())) {
            this.client.sendPacket(MaplePacketCreator.updateSkills(newlist));
        }
    }
...
//                 this.client.sendPacket(MaplePacketCreator.updatePetSkill(((Skill) date.getKey()).getId(), ((SkillEntry) date.getValue()).skillevel, ((SkillEntry) date.getValue()).masterlevel, ((SkillEntry) date.getValue()).expiration));
//                             }
...
//                    getClient().sendPacket(MaplePacketCreator.skillCooldown(statss.getSourceId(), 冷却时间));
        ...
        this.lastDOTTime = 0L;
        this.morphCount = 0;
        if ((!GameConstants.is新手职业(this.getJob()))) {
            ...
            if (charms > 0) {
                ...
                long v10 = this.getExp() - (long) (expforlevel * diepercentage);
                if (v10 < 0L) {
                    v10 = 0L;
                }
                this.setExp(v10);
            }
            updateSingleStat(MapleStat.经验, this.getExp());
        }
        this.cancelAllBuffs();
        cancelAllBuffs_();
    }
        ...
        if ((this.party != null) && (this.client.getChannelServer() != null)) {
            ...
            for (MaplePartyCharacter partychar : this.party.getMembers()) {
                if ((partychar != null) && (partychar.getMapid() == getMapId()) && (partychar.getChannel() == channel)) {
                    ...
                    if (other != null) {
                        other.getClient().sendPacket(PartyPacket.updatePartyMemberHP(getId(), this.stats.getHp(), this.stats.getCurrentMaxHp()));
                    }
                }
            }
        }
    ...
    public void receivePartyMemberHP() {
        ...
        for (MaplePartyCharacter partychar : this.party.getMembers()) {
            if ((partychar != null) && (partychar.getMapid() == getMapId()) && (partychar.getChannel() == channel)) {
                MapleCharacter other = this.client.getChannelServer().getPlayerStorage().getCharacterByName(partychar.getName());
                if (other != null) {
                    this.client.sendPacket(PartyPacket.updatePartyMemberHP(other.getId(), other.getStat().getHp(), other.getStat().getCurrentMaxHp()));
                }
            }
        }
    }
    ...
    public void gainExp(int total, boolean show, boolean inChat, boolean white) {
        try {
            ...
            if (this.getLevel() >= ServerProperties.getMaxLevel()) {
                setExp(0L);
            } else {
                boolean leveled = false;
                long tot = this.getExp() + total;
                if (tot >= needed) {
                    this.addExp(total);
                    levelUp();
                    leveled = true;
    ...
    public void gainExpMonster(long gain, boolean 显示, boolean 最高伤害, byte pty, int Class_Bonus_EXP, MapleMonster mob) {
        ...
        if (this.getLevel() >= ServerProperties.getMaxLevel()) {
            ...
            boolean leveled = false;
            if ((this.getExp() + total >= needed) || (this.getExp() >= needed)) {
                this.addExp(total);
                while (this.getExp() > needed) {
                    levelUp();
                    needed = getExpNeededForLevel();
                }
                leveled = true;
        ...
        if (total != 0L) {
            ...
            if (显示) {
                ...
                this.client.sendPacket(MaplePacketCreator.GainEXP_Monster((int) total, 最高伤害, expStatup));
            }
        }
    ...
    public void forceReAddItem(Item item, MapleInventoryType type) {
        ...
        if (type != MapleInventoryType.UNDEFINED) {
            this.client.sendPacket(InventoryPacket.modifyInventory(false, Collections.singletonList(new ModifyInventory(0, item))));
        }
    }
...
//            this.client.sendPacket(MaplePacketCreator.pendantSlot(false));
        ...
        if (!this.pendingExpiration.isEmpty()) {
            for (Integer itemId : this.pendingExpiration) {
                if (ii.isCash(itemId)) {
                    this.client.sendPacket(MaplePacketCreator.showCashItemExpired(itemId));
                } else {
                    this.client.sendPacket(MaplePacketCreator.showItemExpired(itemId));
                }
            }
        }
        ...
        if (!this.pendingSkills.isEmpty()) {
            this.client.sendPacket(MaplePacketCreator.updateSkills(this.pendingSkills));
//            this.client.sendPacket(MaplePacketCreator.showSkillExpired(this.pendingSkills));
        }
    ...
    public void gainMeso(long gain, boolean show, boolean inChat) {
        if (this.getMeso() + gain < 0) {
            this.client.sendPacket(MaplePacketCreator.enableActions());
            return;
        }
        this.getCharPo().setMeso((int)(this.getMeso() + gain));
        if (this.getMeso() > 2147480000) {
            this.getCharPo().setMeso(2147480000);
        }
        updateSingleStat(MapleStat.金币, this.getMeso(), false);
        this.client.sendPacket(MaplePacketCreator.enableActions());
        if (show) {
...
//                    this.client.sendPacket(MaplePacketCreator.getShowQuestCompletion(q.getQuest().getId()));
    ...
    public void levelUp(boolean takeexp) {
        try {
            ...
            if (GameConstants.is新手职业(getJob())) {
                maxhp += Randomizer.rand(12, 16);
                maxmp += Randomizer.rand(10, 12);
            } else if ((getJob() >= 100) && (getJob() <= 132)) {
                maxhp += Randomizer.rand(48, 52);
                maxmp += Randomizer.rand(4, 6);
            } else if ((getJob() >= 200) && (getJob() <= 232))  {
                maxhp += Randomizer.rand(10, 14);
                maxmp += Randomizer.rand(48, 52);
            }else if (((getJob() >= 300) && (getJob() <= 322)) || ((getJob() >= 400) && (getJob() <= 434)))  {
                maxhp += Randomizer.rand(20, 24);
                maxmp += Randomizer.rand(14, 16);
            } else {
                maxhp += Randomizer.rand(24, 38);
                maxmp += Randomizer.rand(12, 24);
                if ((getJob() != 800) && (getJob() != 900) && (getJob() != 910)) {
                    System.err.println(new StringBuilder().append("出现未处理的角色升级加血职业: ").append(getJob()).toString());
                }
            }
            ...
            statup.put(MapleStat.经验, getExp());
            statup.put(MapleStat.等级, Long.valueOf(getLevel()));

            if ((isGM()) || (!GameConstants.is新手职业(getJob()))) {
                remainingSp += 3;
            }
            remainingAp += 5;
...
//                client.sendPacket(PetPacket.petStatUpdate(this));
    ...
    public void unequipSpawnPet(MaplePet pet, boolean shiftLeft, boolean hunger) {
        ...
        this.client.sendPacket(PetPacket.updatePet(pet, getInventory(MapleInventoryType.CASH).getItem((short) (byte) pet.getInventoryPosition()), true));
        if (this.map != null) {
    ...
    public byte getWorld() {
        return this.getCharPo().getWorld();
    }

    public void setWorld(byte world) {
        this.getCharPo().setWorld(world);
    }
    ...
    public boolean modifyCSPoints(int type, int quantity, boolean show) {
        switch (type) {
            case 1:
                if (this.getCharPo().getAccount().getACash() + quantity < 0) {
                    if (show) {
                ...
                getCharPo().getAccount().setACash(getCharPo().getAccount().getACash() + quantity);
                break;
            case 2:
                if (this.getCharPo().getAccount().getPoints() + quantity < 0) {
                    if (show) {
                ...
                getCharPo().getAccount().setPoints(getCharPo().getAccount().getPoints() + quantity);
                break;
        }
...
//        this.client.sendPacket(MaplePacketCreator.showCharCash(this));
    ...
    public int getCSPoints(int type) {
        switch (type) {
            case 1:
                return getCharPo().getAccount().getACash();
            case 2:
                return getCharPo().getAccount().getPoints();
            case -1:
                return getCharPo().getAccount().getACash() + getCharPo().getAccount().getPoints();
            case 0:
        }
        return 0;
    }
    ...
    public void setBuddyCapacity(byte capacity) {
        ...
        this.client.sendPacket(BuddyListPacket.updateBuddyCapacity(capacity));
    }
...
//                client.sendPacket(MaplePacketCreator.skillCooldown(skillId, (int) (冷却时间 / 1000)));
    ...
    public void giveDebuff(MapleDisease disease, int x, long duration, int skillid, int level) {
        if ((this.map != null) && (!hasDisease(disease)) && Randomizer.nextInt(100) > Math.min(this.getStat().getAsrR(), 99)) {
            ...
            this.map.broadcastMessage(this, BuffPacket.giveForeignDebuff(this.getId(), disease, skillid, level, x), false);
    ...
    public boolean dispelDebuff(MapleDisease debuff) {
        if (hasDisease(debuff)) {
            this.client.sendPacket(BuffPacket.cancelDebuff(debuff));
            this.map.broadcastMessage(this, BuffPacket.cancelForeignDebuff(this.getId(), debuff), false);
    ...
    public void checkBerserk() {
        if ((this.getJob() != 132) || (this.lastBerserkTime < 0L) || (this.lastBerserkTime + 10000L > System.currentTimeMillis())) {
            return;
        }
        int skillId = 1320016;
        ...
        if ((skilllevel >= 1) && (this.map != null)) {
            ...
            this.client.sendPacket(MaplePacketCreator.showOwnBuffEffect(skillId, 1, getLevel(), skilllevel, (byte) (this.stats.Berserk ? 1 : 0)));
...
//            this.client.sendPacket(MaplePacketCreator.yellowChat(message));
    ...
    public void dropSpouseMessage(int type, String message) {
        this.client.sendPacket(MaplePacketCreator.serverNotice(5, message));
    }
    ...
    public void setPoints(int p) {
        this.getCharPo().getAccount().setPoints(p);
    }

    public int getPoints() {
        return this.getCharPo().getAccount().getPoints();
    }

    public void setVPoints(int p) {
        this.getCharPo().getAccount().setVpoints(p);
    }

    public int getVPoints() {
        return this.getCharPo().getAccount().getVpoints();
    }
    ...
    public void removeAll(int itemId, boolean show, boolean checkEquipped) {
        ...
        if (possessed > 0) {
            ...
            if (show) {
                getClient().sendPacket(MaplePacketCreator.getShowItemGain(itemId, (short) (-possessed), true));
            }
        }
        if ((checkEquipped) && (type == MapleInventoryType.EQUIP)) {
...
//            this.map.broadcastMessage(MaplePacketCreator.removeExtractor(this.getId()));
...
//        this.cancelAllBuffs();
        World.ChannelChange_Data(this, channel);
    ...
    public void expandInventory(byte type, int amount) {
        ...
        this.client.sendPacket(InventoryPacket.updateInventorySlotLimit(type, inv.getSlotLimit()));
    }
...
//                    updateOneInfo(questid, "date", FileoutputUtil.CurrentReadable_Date());
    ...
    public void resetStatsByJob(boolean beginnerJob) {
        int baseJob = beginnerJob ? this.getJob() % 1000 : this.getJob() % 1000 / 100 * 100;
    ...
    public void startPartySearch(List<Integer> jobs, int maxLevel, int minLevel, int membersNeeded) {
        for (MapleCharacter chr : this.map.getCharacters()) {
            if ((chr.getId() != this.getId()) && (chr.getParty() == null) && (chr.getLevel() >= minLevel) && (chr.getLevel() <= maxLevel) && ((jobs.isEmpty()) || (jobs.contains((int) chr.getJob()))) && ((isGM()) || (!chr.isGM()))) {
    ...
    public void makeBattler(int index, int monsterId) {
        ...
        this.battlers[index].setCharacterId(this.getId());
        this.changed_pokemon = true;
    }
    ...
    public void refreshItem(Item item) {
        ...
        client.sendPacket(InventoryPacket.addItemToInventory(item));
    }
    ...
    public void fakeRelog() {
        ...
        if (isShowPacket()) {
            client.sendPacket(MaplePacketCreator.serverNotice(5, "刷新人物数据完成..."));
        }
    }
...
//         this.client.sendPacket(MaplePacketCreator.sendPolice(text));
//         this.isbanned = true;
//         WorldTimer.getInstance().schedule(new Runnable() {
//            public void run() {
//                 MapleCharacter.this.client.disconnect(true, false);
...
//        }, 6000L);
    ...
    public long getExpNeededForLevel() {
        return GameConstants.getExpNeededForLevel(this.getLevel());
    }
    ...
    public void resetStats(int str, int dex, int int_, int luk, boolean resetAll) {
        ...
        this.client.sendPacket(MaplePacketCreator.updatePlayerStats(stat, false, this));
    }
