...
//        }
        ...
        if (derp == null) {
            MapleLogger.info("找不到怪物的掉落："+mob.getId());
            return;
        }
...
//                }
                if (de.itemId > 0) {
                    ...
                    idrop.setGMLog(new StringBuilder().append("怪物掉落: ").append(mob.getId()).append(" 地图: ").append(this.mapid).append(" 时间: ").append(System.currentTimeMillis()).toString());
        ...
        for (MonsterGlobalDropEntry de : globalEntry) {
            ...
            if ((Randomizer.nextInt(GameConstants.DROP_ITEM_PER) < de.chance * globalServerRate) && ((de.continent < 0) || ((de.continent < 10) && (this.mapid / 100000000 == de.continent)) || ((de.continent < 100) && (this.mapid / 10000000 == de.continent)) || ((de.continent < 1000) && (this.mapid / 1000000 == de.continent)))) {
                if ((de.itemId == 0) && (cashServerRate != 0)) {
                    ...
                    idrop.setGMLog(new StringBuilder().append("怪物掉落: ").append(mob.getId()).append(" 地图: ").append(this.mapid).append(" (Global) 时间: ").append(System.currentTimeMillis()).toString());
                    if (ItemConstants.isNoticeItem(de.itemId)) {
                        broadcastMessage(MaplePacketCreator.serverMessageNotice(new StringBuilder(127).append("\"").append(chr.getName()).append("\" 在\"").append(chr.getMap().getMapName()).append("\"杀死\" ").append(mob.getStats().getName()).append("\"，掉落道具\"").append(ii.getName(de.itemId)).append("\"").toString()));
                    }
...
//                    chr.getClient().sendPacket(UIPacket.MapEff("monsterPark/clearF"));
            ...
            switch (chr.getMapId() % 1000 / 100) {
                case 1:
                case 2:
//                    chr.getClient().sendPacket(MaplePacketCreator.showEffect("aswan/clear"));
//                    chr.getClient().sendPacket(MaplePacketCreator.playSound("Party1/Clear"));
                    break;
                case 3:
//                    chr.getClient().sendPacket(MaplePacketCreator.showEffect("aswan/clearF"));
//                    chr.getClient().sendPacket(MaplePacketCreator.playSound("Party1/Clear"));
...
//                MapleLogger.info("停止控制 " + monster.getObjectId()  +"  tr: "+Controller.getTruePosition().distance(monster.getTruePosition())+"  "+monster.getRange());
...
//            MapleLogger.info("新控制 " + monster.getObjectId());
    ...
    public void spawnRevives(final MapleMonster monster, final int oid) {
        ...
        spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
            ...
            public void sendPackets(MapleClient c) {
                c.sendPacket(MobPacket.spawnMonster(monster, monster.getStats().getSummonType() <= 1 ? -3 : monster.getStats().getSummonType(), oid));
            }
        });
    ...
    public void spawnMist(final MapleDefender mist, final int duration, boolean fake) {
        ...
        if ((mist.isPoisonMist()) && (!mist.isMobMist())) {
            ...
            if (mist.isRecoverMist()) {
                ...
                MapleLogger.info("[未处理 mist] 来自 " + from + " 技能ID：" + id + "");
                poisonSchedule = null;
            }
        }
    ...
    public void spawnMesoDrop(int meso, Point position, final MapleMapObject dropper, MapleCharacter owner, boolean playerDrop, byte droptype) {
        ...
        spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
            @Override
            public void sendPackets(MapleClient c) {
                c.sendPacket(InventoryPacket.dropItemFromMapObject(mdrop, dropper.getTruePosition(), droppos, (byte) 1, false));
            }
        });
        if (!this.everlast) {
    ...
    public void spawnMobMesoDrop(int meso, final Point position, final MapleMapObject dropper, MapleCharacter owner, boolean playerDrop, byte droptype) {
        ...
        spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
            @Override
            public void sendPackets(MapleClient c) {
                c.sendPacket(InventoryPacket.dropItemFromMapObject(mdrop, dropper.getTruePosition(), position, (byte) 1, false));
            }
        });
        mdrop.registerExpire(120000L);
        if ((droptype == 0) || (droptype == 1)) {
        ...
        spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
            ...
            public void sendPackets(MapleClient c) {
                if ((c != null) && (c.getPlayer() != null) && ((questid <= 0) || (c.getPlayer().getQuestStatus(questid) == 1)) && ((idrop.getItemId() / 10000 != 238) ) && (mob != null) && (dropPos != null)) {
                    c.sendPacket(InventoryPacket.dropItemFromMapObject(mdrop, mob.getTruePosition(), dropPos, (byte) 1, false));
                }
            }
        });
        mdrop.registerExpire(120000L);
    ...
    public void spawnAutoDrop(int itemid, final Point pos) {
        ...
        spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
            @Override
            public void sendPackets(MapleClient c) {
                c.sendPacket(InventoryPacket.dropItemFromMapObject(mdrop, pos, pos, (byte) 1, false));
            }
        });
    ...
    public void spawnItemDrop(final MapleMapObject dropper, MapleCharacter owner, Item item, Point pos, boolean ffaDrop, boolean playerDrop) {
        ...
        spawnAndAddRangedMapObject(drop, new DelayedPacketCreation() {
            @Override
            public void sendPackets(MapleClient c) {
                c.sendPacket(InventoryPacket.dropItemFromMapObject(drop, dropper.getTruePosition(), droppos, (byte) 1, false));
            }
        });
...
//        GameConstants.achievementRatio(chr.getClient());
        if ((GameConstants.isTeamMap(this.mapid)) ) {
            //chr.getClient().sendPacket(MaplePacketCreator.showEquipEffect(chr.getTeam()));
        }

        MaplePet pets = chr.getSpawnPets(); //宠物发的包
        if ((pets != null) && (pets.getSummoned())) {
            pets.setPos(chr.getTruePosition());
//            chr.getClient().sendPacket(PetPacket.updatePet(pets, chr.getInventory(MapleInventoryType.CASH).getItem((short) (byte) pets.getInventoryPosition()), false));
            chr.getClient().sendPacket(PetPacket.showPet(chr, pets, false, false, true));
//            chr.getClient().sendPacket(PetPacket.loadExceptionList(chr, pets));
        }
...
//            chr.getClient().sendPacket(PartyPacket.updateParty(chr.getClient().getChannel(), chr.getParty(), PartyOperation.更新队伍, null));
        ...
        if ((!chr.isInBlockedMap()) && (chr.getLevel() >= 10)) {
            for (MapleQuickMove qm : MapleQuickMove.values()) {
                if (qm.getMap() == chr.getMapId()) {
                    ...
                    quickMove = true;
//                    chr.getClient().sendPacket(MaplePacketCreator.getQuickMoveInfo(true, qmn));
                    break;
                }
            }
        }
        if (!quickMove) {
//            chr.getClient().sendPacket(MaplePacketCreator.getQuickMoveInfo(false, new LinkedList()));
        }
...
//        }
        ...
        if ((getSquadBegin() != null) && (getSquadBegin().getTimeLeft() > 0L) && (getSquadBegin().getStatus() == 1)) {
            chr.getClient().sendPacket(MaplePacketCreator.getClock((int) (getSquadBegin().getTimeLeft() / 1000L)));
        }
...
//                                chr.getClient().sendPacket(packet);
    ...
    public void addAreaMonsterSpawn(MapleMonster monster, Point pos1, Point pos2, Point pos3, int mobTime, String msg, boolean shouldSpawn, boolean sendWorldMsg) {
        ...
        if ((pos1 == null) && (pos2 == null) && (pos3 == null)) {
            MapleLogger.info(new StringBuilder().append("WARNING: mapid ").append(this.mapid).append(", monster ").append(monster.getId()).append(" could not be spawned.").toString());
            return;
        }
        if (pos1 != null) {
            if (pos2 == null) {
